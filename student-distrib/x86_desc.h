...
/*page directory for 4kB- the elements of the
 struct are as defined in the documentation
 and each struct has been assigned the number of
 bits that belong to that flag*/
...
/*page directory for 4MB- the elements of the
 struct are as defined in the documentation
 and each struct has been assigned the number of
 bits that belong to that flag*/
...
/*struct for a general page directory*/
typedef union PDE_MB_KB_t
{
    PDE_MB_t PDE_M;
    PDE_KB_t PDE_K;
}PDE_MB_KB_t;

/*struct for final page directory of size 1024*/
typedef struct PDE_final_t
{
    PDE_MB_KB_t PDE[oneK];
}PDE_final_t;

/*page table for 4kB- the elements of the
 struct are as defined in the documentation
 and each struct has been assigned the number of
 bits that belong to that flag*/
typedef union PTE_KB_t
{
  uint32_t PTE_val;
  struct{
    uint8_t present :1;
...
/*creating array for 8 page durectories pf type paage directory*/
PDE_final_t page_dir[8]__attribute__((aligned(0x1000)));
/*creating array of 1024 for page tables*/
PTE_KB_t page_table[oneK]__attribute__((aligned(0x1000)));
